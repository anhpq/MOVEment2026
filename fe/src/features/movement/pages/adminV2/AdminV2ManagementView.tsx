...
export function AdminV2ManagementView() {
  ...
  const formatQrStatus = (status: string | undefined) => {
    if (!status) return t("systemConfig.qrStatus.loading");
    const activeCount = /^ACTIVE x(\d+)$/.exec(status);
    if (activeCount) {
      return t("systemConfig.qrStatus.activeCount", {count: Number(activeCount[1])});
    }
  ...
  useEffect(() => {
    if (teams.length === 0 && stationDefinitions.length === 0) return;
  ...
  const openTeamProgress = (teamId: string) => {
    setActiveTeam(teamId);
    navigate(`/teams/${teamId}/stations`);
  };

  return (
    <section className="admin-v2-management" style={{padding: "16px 0"}}>
      <Tabs
        ...
        items={[
              ...
              <Flex vertical gap={16} className="full-width">
                ...
                <List
                  ...
                  renderItem={(team) => {
                    ...
                    return (
                      <List.Item>
                        <Card className="surface-card station-card" style={{width: "100%"}}>
                            ...
                            <Flex vertical gap={4} className="full-width">
                              <Flex align="center" gap={8} className="full-width">
                                ...
                                <Button
                                  type="default"
                                  icon={<EyeOutlined />}
                                  title="Xem tiến độ trạm"
                                  onClick={() => openTeamProgress(team.id)}>
                                  Tiến độ
                                </Button>
                                <Button
                                  shape="circle"
                                  ...
                                  icon={<EditOutlined />}
                                  aria-label={t("systemConfig.editTeamAria", {team: teamName})}
                                  title={t("systemConfig.editTeamAria", {team: teamName})}
                                  onClick={() => navigate(`/system-config/teams/${team.id}`)}
                                />
                                <Button
                                  shape="circle"
                                  color="danger"
                                  ...
                                  className="delete-icon-button"
                                  icon={<DeleteOutlined />}
                                  aria-label={t("systemConfig.deleteTeamAria", {team: teamName})}
                                  title={t("systemConfig.deleteTeamAria", {team: teamName})}
                                  onClick={() => {
                                    modal.confirm({
                                      centered: true,
                                      ...
                                      onOk: async () => {
                                        try {
                                          ...
                                          loadDatabase(await fetchAdminDatabase());
                                          message.success(t("systemConfig.teamDeleted"));
                                        } catch (error) {
                                          message.error(t("systemConfig.deleteTeamFailed"));
                                          throw error;
                                        }
                                      },
                                    });
          ...
          {
            key: "stations",
            label: `${t("systemConfig.stationList", {count: stationDefinitions.length})}`,
                            ...
                            <Flex vertical gap={4} className="full-width">
                              <Flex gap={8} className="full-width">
                                ...
                                <Button
                                  ...
                                  className="delete-icon-button"
                                  icon={<EditOutlined />}
                                  aria-label={t("systemConfig.editStationAria", {station: stationDisplay.name})}
                                  title={t("systemConfig.editStationAria", {station: stationDisplay.name})}
                                  onClick={() => navigate(`/system-config/stations/${station.id}`)}
                                />
                                <Button
                                  shape="circle"
                                  color="danger"
                                  ...
                                  className="delete-icon-button"
                                  icon={<DeleteOutlined />}
                                  aria-label={t("systemConfig.deleteStationAria", {station: stationDisplay.name})}
                                  title={t("systemConfig.deleteStationAria", {station: stationDisplay.name})}
                                  onClick={() => {
                                    modal.confirm({
                                      centered: true,
                                      ...
                                      onOk: async () => {
                                        try {
                                          ...
                                          loadDatabase(await fetchAdminDatabase());
                                          message.success(t("systemConfig.stationDeactivated"));
                                        } catch (error) {
                                          message.error(t("systemConfig.deleteStationFailed"));
                                          throw error;
                                        }
                                      },
                                    });
                              ...
                              <Select
                                ...
                                onChange={(value) => void handleTrackingModeChange(station, value)}
                              />
                            </Flex>
                            <Flex gap={8} className="station-actions" wrap align="center">
