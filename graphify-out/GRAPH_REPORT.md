...
## Corpus Check
- 131 files · ~153,332 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1074 nodes · 2072 edges · 99 communities (73 shown, 26 thin omitted)
- Extraction: 100% EXTRACTED · 0% INFERRED · 0% AMBIGUOUS · INFERRED: 2 edges (avg confidence: 0.65)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `3013c18c`
...
1. `AuthContext` - 41 edges
2. `AdminService` - 40 edges
3. `CurrentAuth` - 33 edges
4. `PrismaService` - 33 edges
5. `AdminController` - 29 edges
6. `useMovementStore` - 27 edges
7. `PlayerService` - 25 edges
8. `FinalService` - 22 edges
9. `AuthService` - 20 edges
10. `Backend Audit Status` - 20 edges
...
Cohesion: 0.13
Nodes (26): getPlayerDashboard(), getPlayerProgress(), getPlayerStations(), loginTeam(), loginTeamWithQr(), loginUser(), loginWithQrToken(), PlayerProgressResponse (+18 more)
...
Cohesion: 0.09
Nodes (26): normalizeQrToken(), AuthController, mockAuthService, Body, Controller, Post, AuthService, PrismaSessionClient (+18 more)
...
Cohesion: 0.17
Nodes (20): checkInStation(), editAdminProgressScore(), forceAdminProgressStatus(), buildFallbackPositions(), buildMarkerPosition(), clampMapScale(), clampPercent(), getMarkerFill() (+12 more)
...
Cohesion: 0.15
Nodes (6): EventConfigController, Controller, EventConfigService, Injectable, PrismaService, Injectable
...
### Community 18 - "api.ts"
Cohesion: 0.15
Nodes (24): AdminProgressMatrixResponse, AdminQrLoginTokenResponse, AdminStationUpdateInput, AdminTeamResponse, AuthMeResponse, downloadAdminSummary(), FinalSubmissionResponse, getAdminActivityLogs() (+16 more)
...
Cohesion: 0.21
Nodes (13): displayStatus(), fetchAdminDatabase(), createAdminStation(), createAdminTeam(), getAdminProgressMatrix(), updateAdminStation(), updateAdminTeam(), StationEditorPage() (+5 more)
...
Cohesion: 0.26
Nodes (10): App(), getMe(), logout(), ROLE_LABELS, useMovementBootstrap(), AppFrame(), AppFrameProps, ProtectedRoute() (+2 more)
...
Cohesion: 0.07
Nodes (45): STATUS_ORDER, buildFinishedTeamStations(), buildPatchedTeamStations(), buildResetTeamStations(), buildStartedTeamStations(), buildTeamStationsWithoutStation(), buildTeamStationsWithUpdatedStation(), createFinishedStation() (+37 more)
...
Cohesion: 0.20
Nodes (12): GenerateQrLoginTokenDto, IsInt, IsOptional, Max, Min, CreateTeamDto, IsOptional, IsString (+4 more)
...
Cohesion: 0.23
Nodes (9): FinalResponse, getPlayerFinal(), submitFinalAnswer(), FinalFormValues, FinalPage(), getRemainingSeconds(), StationsMapPage(), TeamListPage() (+1 more)

### Community 77 - "package.json"
Cohesion: 0.27
Nodes (9): main(), prisma, stations, teamColors, teams, buildStationQrToken(), buildTeamLoginQrToken(), createQrTokenFingerprint() (+1 more)
...
### Community 92 - "SystemConfigPage.tsx"
Cohesion: 0.46
...
### Community 94 - "player.service.ts"
Cohesion: 0.62
Nodes (4): QrActionDto, SubmitCipherDto, IsString, MinLength

### Community 95 - "LeaderboardController"
Cohesion: 0.33
...
## Knowledge Gaps
- **352 isolated node(s):** `deploy.sh script`, `NODE_ENV`, `config`, `$schema`, `collection` (+347 more)
...
- **Why does `AuthContext` connect `AuthContext` to `admin.service.ts`, `store.ts`, `FinalService`, `jwt-auth.guard.ts`, `player.service.ts`?**
  _High betweenness centrality (0.024) - this node is a cross-community bridge._
- **Why does `PrismaService` connect `useMovementStore` to `prisma.module.ts`, `store.ts`, `PrismaService`, `package.json`, `FinalService`, `AppFrame.tsx`, `.getPublicConfig`, `xlsx-report.ts`, `jwt-auth.guard.ts`, `player.service.ts`?**
  _High betweenness centrality (0.016) - this node is a cross-community bridge._
- **Why does `AdminService` connect `xlsx-report.ts` to `admin.service.ts`, `scripts`, `PrismaService`, `package.json`, `useMovementStore`, `.getPublicConfig`, `UpdateEventConfigDto`?**
